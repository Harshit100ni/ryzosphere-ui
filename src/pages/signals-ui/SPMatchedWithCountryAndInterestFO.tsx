...
import { AlertCircle, Loader2, MapPin, Briefcase } from "lucide-react";
...
const fetchServiceProviders = async (
  ...
  try {
    const response = await axiosConfig.get(
      API_ENDPOINTS.SP_MATCHED_WITH_COUNTRY_INTEREST,
      {
...
const SPMatchedWithCountryAndInterestFO: React.FC = () => {
    ...
    <div className="container p-6 max-w-2xl mx-auto">
      <div className="relative p-6 bg-white shadow-lg rounded-2xl border border-gray-200">
        <h2 className="text-3xl font-semibold text-gray-800 mb-2 flex items-center gap-2">
          🎯 Service Providers Matching Interests & Country
        </h2>
        <p className="text-gray-600 text-sm mb-4">
          Enter a <strong>Family Office</strong> name to find Service Providers
          who share common interests 🌍 in the same country.
        </p>
          ...
          <div className="mt-6">
            {data.data.length === 0 ? (
              <p className="text-gray-500 text-center">
                No matching Service Providers found for{" "}
              ...
              data.data.map((item: any, index: number) => (
                ...
                >
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-1 flex items-center gap-2">
                    <Briefcase size={20} /> {item?.serviceProvider || "N/A"}
                  </h3>
...
export default SPMatchedWithCountryAndInterestFO;
